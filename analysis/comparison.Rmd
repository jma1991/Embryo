...
fct <- fct[apply(tbl, 1, function(row) all(row !=0 ))]
...
for (n in names(sig)) {
  ...
  plt <- lapply(ids, function(x) plotUMAP(sce[, sel], colour_by = x, shape_by = "genotype", by_exprs_values = "logcounts") + ggtitle(x))
