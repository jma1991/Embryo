...
for (n in names(sig)) {
  ...
  plotHeatmap(sce[, sel], features = ids, x = "cluster", colour_columns_by = "cluster", center = TRUE, zlim = c(-1, 1), symmetric = TRUE, cluster_rows = ifelse(length(ids) >= 2, TRUE, FALSE), show_colnames = FALSE)
