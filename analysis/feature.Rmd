...
metadata(sce)$modelGeneVarByPoisson <- sel
```

Plot heatmap of gene expression values:

```{r}
plotHeatmap(sce, metadata(sce)$modelGeneVarByPoisson, center = TRUE, zlim = c(-1, 1), symmetric = TRUE, treeheight_row = 0, treeheight_col = 0, show_rownames = FALSE, show_colnames = FALSE)
```

### Correlated genes

```{r}
fit <- correlatePairs(sce, subset.row = metadata(sce)$modelGeneVarByPoisson)

sig <- subset(fit, FDR < 0.05)
...
```{r}
plotHeatmap(sce, metadata(sce)$correlatePairs, center = TRUE, zlim = c(-1, 1), symmetric = TRUE, treeheight_row = 0, treeheight_col = 0, show_rownames = FALSE, show_colnames = FALSE)
```
...
```{r}
plotHeatmap(sce, metadata(sce)$modelGeneCV2, center = TRUE, zlim = c(-1, 1), symmetric = TRUE, treeheight_row = 0, treeheight_col = 0, show_rownames = FALSE, show_colnames = FALSE)
```
...
```{r}
plotHeatmap(sce, metadata(sce)$findElbowPoint, center = TRUE, zlim = c(-1, 1), symmetric = TRUE, treeheight_row = 0, treeheight_col = 0, show_rownames = FALSE, show_colnames = FALSE)
```
...
```{r}
dat <- M3Drop::NBumiConvertData(sce)

fit <- M3Drop::NBumiFitModel(dat)

out <- M3Drop::NBumiFeatureSelectionCombinedDrop(fit, method = "fdr", qval.thresh = 0.05, suppress.plot = FALSE)
...
```{r}
plotHeatmap(sce, metadata(sce)$M3DropFeatureSelection, center = TRUE, zlim = c(-1, 1), symmetric = TRUE, treeheight_row = 0, treeheight_col = 0, show_rownames = FALSE, show_colnames = FALSE)
```
...
sel <- cbind(
  ...
  "Var" = rownames(sce) %in% metadata(sce)$modelGeneVarByPoisson,
...
```{r}
sel <- c("modelGeneVarByPoisson", "correlatePairs", "modelGeneCV2", "findElbowPoint", "M3DropFeatureSelection")
```
...
```{r}
sel <- rownames(sce) %in% metadata(sce)$M3DropFeatureSelection
